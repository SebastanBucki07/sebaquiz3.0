...
export function transformCountriesToQuiz(data: RestCountry[]): QuizCountry[] {
  ...
  return data.map((c, index) => {
    const bordersNames = (c.borders ?? []).map((code) => cca3ToPolishName[code]).filter(Boolean);

    const bordersHint = bordersNames.length ? bordersNames.join(', ') : 'Brak graniczących państw';

    const hints: Hint[] = [
      {
        id: '0',
        label: 'Kontynent',
        content: getPolishRegion(c.region, c.subregion),
        penaltyPercent: 20,
      },
